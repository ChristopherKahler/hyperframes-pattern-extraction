...
- **Transcription engine (default: local).** This fork ships two engines — `local` (WhisperX + pyannote, runs on CPU via `.venv/`, no API) and `scribe` (upstream ElevenLabs). `transcribe_batch.py` defaults to `local`. Use `--engine scribe` only if the user explicitly asks. Scribe requires `ELEVENLABS_API_KEY`; local requires `HF_TOKEN` for diarization (can be omitted with `--no-diarize`). See `FORK-NOTES.md`.
- `ffmpeg` + `ffprobe` on PATH.
- Python deps: `pip install -e ".[local]"` (or upstream `pip install -e .` if running Scribe only).
...
- **`transcribe_local.py <video>`** — single-file local WhisperX+pyannote call. Same JSON shape as Scribe. Cached.
- **`transcribe_batch.py <videos_dir>`** — multi-file transcription. Default `--engine local` (serial, shared model). `--engine scribe` runs 4 parallel API workers.
